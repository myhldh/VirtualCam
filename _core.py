...
class SourceNotLoadError(Exception):
    pass
class VirCam:
    def __init__(self,width:int=320,height:int=180,fps:float=30,kill_keys:str='esc',fmt:PixelFormat=PixelFormat.RGB,device:str='Unity Video Capture'):
        '''
        Args:
...
class VirCamUI(VirCam):
    ...
    def show(self,sv:list=None):
        ...
        for i in range(len(self.txts)):
            e.append(Entry(page))
            e[i].config(textvariable=sv[i],width=35)
    ...
    def manager(self):
        if self.manageropened:
            return
        self.manageropened=True
        self.mpage=Toplevel(self.win)
        self.mpage.title('VirCam Manager')
        self.centerwin(self.mpage,270,125)
        self.mpage.focus_set()
        f=Frame(self.mpage)
        scb=Scrollbar(f,orient='vertical')
        scb.pack(fill='y',side='right')
        listbox=Listbox(f,yscrollcommand=scb.set,height=5,width=35,selectmode='single',selectbackground='green',selectforeground='white')
        listbox.pack()
        scb.config(command=listbox.yview)
        for cam in self.running:
            listbox.insert(END,cam)
        f.grid(row=0,column=0,columnspan=2)
        self.managercanclose=True
        def close():
            k=listbox.curselection()
            self.managercanclose=False
            if len(k)==0:
                ...
                while cam in self.running:
                    sleep(0.1)
                self.mpage.focus_set()
            self.managercanclose=True
        def closing():
            if self.managercanclose:
                Thread(target=close,daemon=False).start()
        def closeall():
            for k in self.doing.keys():
                self.doing[k]=False
            ...
            self.managercanclose=True
        def closingall():
            Thread(target=closeall,daemon=False).start()
        bt1=Button(self.mpage,text='Close',fg='green',command=closing)
        bt2=Button(self.mpage,text='Close All',fg='green',command=closingall)
        ...
        def onclose():
            if self.managercanclose:
                self.mpage.destroy()
                self.mpage=None
                self.manageropened=False
        self.mpage.protocol('WM_DELETE_WINDOW',onclose)
        self.mpage.mainloop()
    def winonclose(self):
        if len(self.doing.keys())>0:
            mb.showwarning('Warning','Must close all virtual camera before quitting.')
            self.win.focus_set()
            if not self.mpage:
                self.manager()
            else:
    ...
    def init(self):
        ...
        self.win.protocol('WM_DELETE_WINDOW',self.winonclose)
    def run(self):
        self.top(self.win)
        self.win.focus_set()
        self.preview()
